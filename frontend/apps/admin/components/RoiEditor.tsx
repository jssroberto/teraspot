...
export function RoiEditor({ deviceId }: RoiEditorProps) {
  ...
  useEffect(() => {
    loadInitialData();
  }, [deviceId]); // loadInitialData depends on deviceId, but it's defined inside component. Ideally move it out or use useCallback. For now, just adding deviceId is safe enough as loadInitialData uses it.

  const loadInitialData = async () => {
    setLoading(true);
    try {
      const screenshotData = await triggerScreenshot(deviceId);
      const downloadUrl = screenshotData.download_url;

      // Wait a bit for the image to be available (hacky but from original code)
      ...
      const existingSpaces = await getRoiConfig(deviceId);
  ...
  const finishPolygon = () => {
    ...
    setPolygons([
      ...polygons,
      { space_id: newSpaceId, polygon: currentPoints },
    ]);
    setCurrentPoints([]);
  };
  ...
  const handleSave = async () => {
    setLoading(true);
    try {
      await saveRoiConfig(deviceId, polygons);
      await reloadConfig(deviceId);
    ...
    <View style={styles.container}>
      <View style={styles.toolbar}>
        ...
        <View style={styles.zoomControls}>
          ...
          <Button
            title="+"
            onPress={() => setZoomScale(Math.min(4, zoomScale + 0.5))}
          />
        </View>
      </View>
      ...
      <View style={styles.footer}>
        ...
        />
        <Button
          title="Del Last Shape"
          onPress={() => setPolygons(polygons.slice(0, -1))}
