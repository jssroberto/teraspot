  ...
  const fetchFacilities = async () => {
    try {
      ...
      const uniqueFacilities = Array.from(
        new Set(status.map((s) => s.facility_id).filter(Boolean))
      ) as string[];
              ...
              >
                <Text
                  style={[
                    ...
                    selectedFacility === fac && styles.dropdownItemTextActive,
                  ]}
                >
                  {fac}
                </Text>
        ...
        {menuItems.map((item) => {
          ...
          return (
            <Link key={item.path} href={item.path as any} asChild>
              ...
              >
                <View style={{ marginRight: 12, opacity: isActive ? 1 : 0.7 }}>
                  <IconSymbol
