...
class TestKPILambda:
    ...
    def mock_current_table_data(self):
        ...
        return [
            ...
            },
            {
                ...
                'timestamp': (now - timedelta(minutes=10)).isoformat(),  # Inactive device
    ...
    def test_get_current_occupancy_rate_normal(self, mock_table, mock_current_table_data):
    ...
    @patch('lambda_function.current_table')
    def test_get_average_detection_confidence_excellent(self, mock_table, mock_current_table_data):
    ...
    @patch('lambda_function.current_table')
    def test_get_low_confidence_event_rate_normal(self, mock_table, mock_current_table_data):
    ...
    def test_get_low_confidence_event_rate_action_required(self, mock_table):
        ...
        mixed_data = [
            {'space_id': f'S{i}', 'confidence': Decimal('0.65' if i % 2 == 0 else '0.95'), 
             'timestamp': now.isoformat()}
            for i in range(20)
        ]
    ...
    @patch('lambda_function.current_table')
    def test_get_system_health_device_uptime_healthy(self, mock_table, mock_current_table_data):
        ...
        degraded_data = [
            {
                ...
                'timestamp': (now - timedelta(minutes=10 if i % 2 == 0 else 1)).isoformat()
            }
            for i in range(10)
        ]
    ...
    def test_calculate_average_parking_duration(self, mock_table, mock_history_table_data):
        ...
        assert result['sample_size'] > 0
        assert result['usage_type'] in ['QUICK_ERRANDS', 'RETAIL', 'OFFICE_COMMUTERS', 'AIRPORT_LONG_TERM']
    ...
    def test_handle_malformed_timestamps(self, mock_table):
        ...
        now = datetime.now(timezone.utc)
        malformed_data = [
            {'space_id': 'S1', 'timestamp': 'invalid-timestamp', 'confidence': Decimal('0.9')},
            {'space_id': 'S2', 'timestamp': now.isoformat(), 'confidence': Decimal('0.95')},
        ]
