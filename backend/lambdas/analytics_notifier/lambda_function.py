...
def lambda_handler(event, context):
    """
    Procesa eventos de DynamoDB Streams y loguea la 
    imagen antigua y la nueva en eventos de MODIFICACIÓN.
    """
    
    logger.info(f"Función disparada. Recibidos {len(event.get('Records', []))} registros.")
    
    for record in event.get('Records', []):
        
        # 1. Filtramos solo por eventos de MODIFICACIÓN
        if record['eventName'] == 'MODIFY':
            
            logger.info(f"Procesando evento de MODIFICACIÓN: {record['eventID']}")
            
            try:
                # 2. Obtenemos las imágenes (vienen en formato DynamoDB JSON)
                old_image = record['dynamodb'].get('OldImage', {})
                new_image = record['dynamodb'].get('NewImage', {})

                if not old_image or not new_image:
                    logger.warning(f"Falta OldImage o NewImage. "
                                   f"Por favor, asegúrate de que el Stream esté en modo 'New and old images'.")
                    continue

                # 3. ¡AQUÍ LOGUEAMOS AMBAS IMÁGENES!
                # Usamos json.dumps para formatear el diccionario como un string legible
                ...
                logger.error(f"Error procesando el registro: {str(e)}")
        
        else:
            # Si no es MODIFY (ej. INSERT o DELETE), solo lo registramos
            logger.info(f"Ignorando evento: {record['eventName']}")

    return {
        'statusCode': 200,
        'body': json.dumps('Procesamiento de logs completado')
    }
