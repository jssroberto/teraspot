...
resource "aws_api_gateway_deployment" "deployment" {
  ...
  triggers = {
    redeployment = sha1(jsonencode([
      ...
      aws_api_gateway_method.options_kpi.id,
      aws_api_gateway_integration.options_kpi_integration.id,
      timestamp()
    ]))
  }
...
# CORS for /kpi
resource "aws_api_gateway_method" "options_kpi" {
  rest_api_id   = aws_api_gateway_rest_api.api.id
  resource_id   = aws_api_gateway_resource.kpi_resource.id
  http_method   = "OPTIONS"
  authorization = "NONE"
...
resource "aws_api_gateway_integration" "options_kpi_integration" {
  rest_api_id = aws_api_gateway_rest_api.api.id
  resource_id = aws_api_gateway_resource.kpi_resource.id
  http_method = aws_api_gateway_method.options_kpi.http_method
  type        = "MOCK"
  request_templates = {
    "application/json" = "{\"statusCode\": 200}"
  }
...
resource "aws_api_gateway_method_response" "options_kpi_response" {
  rest_api_id = aws_api_gateway_rest_api.api.id
  resource_id = aws_api_gateway_resource.kpi_resource.id
  http_method = aws_api_gateway_method.options_kpi.http_method
  status_code = "200"
...
resource "aws_api_gateway_integration_response" "options_kpi_integration_response" {
  rest_api_id = aws_api_gateway_rest_api.api.id
  resource_id = aws_api_gateway_resource.kpi_resource.id
  http_method = aws_api_gateway_method.options_kpi.http_method
  status_code = aws_api_gateway_method_response.options_kpi_response.status_code
  ...
  depends_on = [aws_api_gateway_method_response.options_kpi_response]
}
